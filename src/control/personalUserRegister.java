package control;

import frombean.PersonRegisterBean;
import service.PersonalUserService;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;

/**
 * Created by cxspace on 16-7-4.
 */

public class personalUserRegister extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

        request.setCharacterEncoding("utf-8");

        //如果验证失败
        if (!PersonalUserService.Verify(request)){
            request.getRequestDispatcher("/front/register_fail.jsp").forward(request,response);
        }
        else {

            //添加数据到formbean
            PersonRegisterBean personRegisterBean = PersonalUserService.addToFormbean(request);

            //把form里面数据注入数据库
            if (!PersonalUserService.addPersnalUser(personRegisterBean)) {
                request.getRequestDispatcher("/front/systemError.jsp").forward(request, response);
            } else {
                request.getRequestDispatcher("/front/register_success.jsp").forward(request, response);
                //   request.getRequestDispatcher("/front/index.jsp").forward(request,response);
            }

        /*
        System.out.println(userName+"-"+gender+"-"+majorBusinesses+"-"+phone+"-"+finalPhone+"-"+weiXin+"-"+qq);

        System.out.println(detailAddress+"-"+password+"-"+confirmPassword);

        System.out.println(province+"-"+city+"-"+area);

        System.out.println(birthdayYear+"-"+birthdayMonth+"-"+birthdayDay);
       */
        }
    }

    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
          doPost(request,response);
    }
}
